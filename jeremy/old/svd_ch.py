...
class Opening(Scene):
    def construct(self):
        real = TexMobject("\\text{视频中 }~","A\\in\\R^{m\\times n}",".").scale(1.5).set_color_by_tex('A', YELLOW)
        complex = TexMobject("\\text{若 }~","A\\in\\C^{m\\times n}","\\colon","A^T\\to A^H=\\overline{A^T}.").scale(1.5)
...
class Rotation(LinearTransformationScene):
    ...
    def construct(self):
        ...
        title = VGroup(
            TextMobject("\\heiti 旋转:", color=YELLOW, stroke_width=2).add_background_rectangle(),
...
class Scaling(LinearTransformationScene):
    ...
    def construct(self):
        a = np.diag([2, 0.5])
        title = VGroup(
            TextMobject("\\heiti 缩放:", color=YELLOW, stroke_width=2).add_background_rectangle(),
...
class Definition(Scene):
    def construct(self):
        ...
        ortho1 = VGroup(ortho1, TextMobject("正交").next_to(ortho1, DOWN))
        ortho2 = Brace(V_label[0][0], DOWN)
        ortho2 = VGroup(ortho2, TextMobject("正交").next_to(ortho2, DOWN))
        ...
        arrows = VGroup(*[Line(ORIGIN, DOWN * 0.4).add_tip(tip_length=.2) for _ in range(3)])
        ...
        singular = TextMobject("\\kaishu 奇异值").next_to(S, UP, buff=.5)
        ...
        svd = VGroup(
            TextMobject("Singular", " Value", " Decomposition", stroke_width=3),
            TextMobject("\\textbf{\\heiti 奇异值分解}").scale(.8)
        ).arrange(DOWN).scale(1.5).to_edge(DOWN, buff=.5)
        self.play(Write(svd))
        self.wait()
...
class SingularValuesRank(Scene):
    def construct(self):
        ...
        same_rank = TextMobject("他俩有相同的秩", color=BLUE).next_to(braces, DOWN)
...
class ExpansionExample(Scene):
    def construct(self):
        ...
        self.wait()
        outer_product = VGroup(*[TextMobject("外积").next_to(braces[i], DOWN) for i in range(3)])
...
class LowRankApprox(Scene):
    def construct(self):
        ...
        Ak = VGroup(
            ...
            TextMobject("的秩是 $k$", "~$(k<r)$,"),
        ).arrange().to_edge(UP, buff=.2)
        ...
        nearest = TextMobject(r"并且是离$A$", r" ``最近的''", r"~$k$秩矩阵.") \
            .set_color_by_tex('近', YELLOW).next_to(Ak, DOWN)
        ...
        sol = TexMobject("\\Longrightarrow~", "X", "=", "A_k", "\\text{~ 是一个解.}").tm(m).next_to(prob, DOWN,
                                                                                                         buff=.5)
        self.play(Write(sol))
        self.wait()
        ...
        name = VGroup(
            TextMobject("low-rank approximation", color=YELLOW, stroke_width=2),
            TextMobject("\\textbf{\\heiti 低秩近似}", color=YELLOW).scale(.8)
        ).arrange(DOWN).scale(1).to_edge(DOWN, buff=.4)
...
class Calc(Scene):
    def construct(self):
        ...
        title = Title("\\textbf{\\heiti SVD的计算}")
        self.add(title)
        self.wait()
        ...
        results = VGroup(
            TexMobject("\\sigma_i", "=", "\\sqrt{\\text{$AA^T$ 或 $A^TA$的特征值}}").set_color_by_tex('sigma', YELLOW),
            TexMobject("u_i", "=", "\\text{$AA^T$的特征向量 (正交单位化)}").set_color_by_tex('u_i', RED),
            TexMobject("v_i", "=", "\\text{$A^TA$的特征向量 (正交单位化)}").set_color_by_tex('v_i', GREEN),
        ).arrange(DOWN, aligned_edge=LEFT).next_to(AT_A, DOWN, buff=.8, aligned_edge=LEFT)
        VGroup(
            results[0][-1][2:5],results[0][-1][6:9], results[1][-1][:3], results[2][-1][:3]
        ).set_color(YELLOW)
...
class CalcExample(Scene):
    ...
    def construct(self):
        ...
        a = np.array([[1,2],[2,0],[0,2]])
        A = Matrix(a, h_buff=.9).scale(.8)
        A.add(TexMobject(",").next_to(A))
        prob = VGroup(
            VGroup(TexMobject("A","="), A).arrange().set_color(YELLOW),
            TexMobject("AA^T", "=", "U", "\\Sigma^2", "U^T",",").tm(m),
            TexMobject("A^TA", "=", "V", "\\Sigma^2", "V^T").tm(m),
        ).arrange().to_edge(UP)
        prob[-1].align_to(prob[-2], UP)
        A[-1].set_color(WHITE).align_to(prob[-2][-1], DOWN)
        self.play(Write(prob[0]))
        self.wait()
        self.play(Write(prob[1:]))
        self.wait()
        ...
        ATA = VGroup(
            TexMobject("A^TA="),
            Matrix(a.T.dot(a), h_buff=.9),
            TextMobject("的特征值:"," $9,4$")
        ).arrange()#.next_to(prob, DOWN)
        Sigma = VGroup(
            TexMobject("~~\\Longrightarrow~~", color=BLUE),
            TexMobject("\\Sigma=", color=YELLOW),
            Matrix(np.array([[3,0],[0,2],[0,0]]), h_buff=.9).scale(.8).set_color(YELLOW)
        ).arrange()#.next_to(ATA, DOWN, index_of_submobject_to_align=1)
        sigma = VGroup(ATA, Sigma).arrange().next_to(prob, DOWN)
        self.play(Write(ATA[0:2]))
        self.play(Write(ATA[2][0]))
        self.wait()
        self.play(Write(ATA[2][1:]))
        self.wait()
        self.play(Write(Sigma))
        self.wait()

        v12 = VGroup(
            TextMobject("标准正交的特征向量:"),
            TexMobject("v_1={1\\over\\sqrt5}\\begin{bmatrix}1\\\\2\\end{bmatrix},").set_color(GREEN),
            TexMobject("v_2={1\\over\\sqrt5}\\begin{bmatrix}2\\\\-1\\end{bmatrix}").set_color(GREEN),
        ).arrange().next_to(sigma, DOWN)
        self.play(Write(v12))
        self.wait()
        trick = VGroup(
            TexMobject("A","=","U","\\Sigma"," V^T").tm(m),
            TexMobject("\\Longrightarrow", color=BLUE),
            TexMobject("A","V","=","U","\\Sigma").tm(m),
            TexMobject("\\Longrightarrow", color=BLUE),
            TexMobject("A","v_i","=","\\sigma_i","u_i").tm(m),
            TexMobject("~(","A^T","u_i","=","\\sigma_i","v_i",")").tm(m),
        ).arrange().to_edge(DOWN)
        self.play(Write(trick[:2]))
        self.wait()
        self.play(Write(trick[2:4]))
        self.wait()
        self.play(Write(trick[4:-1]))
        self.wait()
        self.play(Write(trick[-1]))
        self.wait()

        self.play(FadeOut(trick[-1]), FadeOut(trick[:-2]))
        self.play(trick[-2].to_edge, {'edge': LEFT, 'buff': .5})
        self.wait()

        u12 = VGroup(
            TexMobject("\\Longrightarrow", color=BLUE),
            TexMobject("u_1","=","\\frac13", "A", "v_1", ",").tm(m),
            TexMobject("u_2","=","\\frac12", "A", "v_2", ",").tm(m),
            TextMobject("$u_3$","正交于","$u_1$", ",", "$u_2$", ".").tm(m)
        ).arrange().next_to(trick[-2])
        self.play(Write(u12[:-1]))
        self.wait()
        self.play(Write(u12[-1]))
        self.wait()
...
class Pic(Scene):
    ...
    def construct(self):
        ...
        svd = TextMobject("SVD", color=YELLOW, stroke_width=3).scale(1.5).next_to(arrow, UP)
